...
@Slf4j
@Path("/celass")
public class CelassApi {
    @Inject
    private CelassServiceImpl attachService;
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response save(Celass attach) {
        try {
            log.info("Save Celass");
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response edit(Celass attach) {
        try {
            log.info("Edit Celass");
