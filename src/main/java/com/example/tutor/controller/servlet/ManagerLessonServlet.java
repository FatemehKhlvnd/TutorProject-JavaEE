...
@Slf4j
@WebServlet(name = "lessonServlet", urlPatterns = "/Manager-lesson.do")
public class ManagerLessonServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        log.info("LessonServlet - GET");
        response.setContentType("application/json");
        response.setCharacterEncoding("UTF-8");
        try {
            log.info("LessonServlet - find all");
            List<Lesson> lessonList = lessonService.findAll();
            log.info("LessonServlet - print");
            log.info("LessonServlet - setattribute");
            request.getSession().setAttribute("lessonList", lessonList);
            log.info("LessonServlet - setattribute succes");
